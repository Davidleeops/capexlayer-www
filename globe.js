(function(){
  const cities=[
    ...
    {n:"London",lat:51.5,lon:-.12,a:.28},{n:"Dubai",lat:25.2,lon:55.27,a:.22},
    {n:"Singapore",lat:1.35,lon:103.82,a:.22},{n:"Sydney",lat:-33.86,lon:151.2,a:.18}
  ];
  ...
  function mount(canvas){
    ...
    function draw(now){
      ...
      if(arcs.length<7&&Math.random()<.08){
        const visible=cities.map((c,i)=>({i,p:project(c,rot)})).filter(o=>o.p.z>-.1&&cities[o.i].a>.5);
        if(visible.length>1){
          const a=visible[Math.floor(Math.random()*visible.length)].i;
          let b=visible[Math.floor(Math.random()*visible.length)].i;
          if(a!==b)arcs.push({a,b,born:dt,life:2.8});
        }
      }
      for(let i=arcs.length-1;i>=0;i--){
        const ar=arcs[i],age=dt-ar.born;if(age>ar.life){arcs.splice(i,1);continue;}
        const A=project(cities[ar.a],rot),B=project(cities[ar.b],rot);if(A.z<-.25||B.z<-.25)continue;
        const mx=(A.x+B.x)/2,my=(A.y+B.y)/2-r*.24;
        ctx.globalAlpha=Math.sin((age/ar.life)*Math.PI);
        ctx.strokeStyle=gold;ctx.lineWidth=1.4;ctx.beginPath();ctx.moveTo(A.x,A.y);ctx.quadraticCurveTo(mx,my,B.x,B.y);ctx.stroke();
        const q=age/ar.life, x=(1-q)*(1-q)*A.x+2*(1-q)*q*mx+q*q*B.x, y=(1-q)*(1-q)*A.y+2*(1-q)*q*my+q*q*B.y;
        ctx.fillStyle=gold2;ctx.beginPath();ctx.arc(x,y,3.4,0,Math.PI*2);ctx.fill();ctx.globalAlpha=1;
      }
      cities.forEach(c=>{
